...
class DataValidation:
        ...
        try:
            ...
            for num_col in numeric_columns:
                if num_col not in dataframe_columns:
                    missing_numeric_columns.append(num_col)
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            status=self.numeric_columns_exist(dataframe=train_dataframe)
            if not status:
                raise Exception("Train data does not contain all numeric columns")
            status=self.numeric_columns_exist(dataframe=test_dataframe)
            if not status:
                raise Exception("Test data does not contain all numeric columns")
            ##detect data drift
